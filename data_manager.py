...
# 50400 × 1m = 35 дней — достаточно для всех индикаторов (ATR100 на 5m и пр.)
MINI_BARS = 50400

# Полная история для графика — то же значение
CHART_LIMIT = 50400

# Через сколько секунд неактивности выгружать символ из chart_store
CHART_EVICT_SEC = 100     # 10 минут

# Неделя в секундах — старше этого обрезаем из кэша
CACHE_WEEK_SEC  = 36 * 24 * 3600   # 36 дней (чуть больше 50400 минут = 35 дней)

# Параллельность при первоначальной загрузке:
# При ПЕРВОМ запуске (нет кэша) качаем строго последовательно через очередь
# с rate limiting — чтобы не получить 429.
# При наличии кэша — параллельно, т.к. делаем только incremental (1 запрос/символ).
MAX_WORKERS      = 3      # параллельность для incremental-обновлений (есть кэш)
INITIAL_RATE_RPS = 3.0    # запросов/сек при первой загрузке (cold start, нет кэша)
                           # Binance Futures лимит: 2400 req/min = 40 req/sec
                           # 50400 баров = ~51 batch запросов на символ
                           # При 4 rps на весь пул: безопасно, ~14 сек на символ
...
def cache_load(symbol: str, tf: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if df.empty:
            return None
        # FIX #1: Normalize timezone to UTC (handle both localized and non-localized)
...
def fetch_ohlcv_sync(exchange, symbol: str, timeframe: str, limit: int) -> pd.DataFrame:
    ...
    all_data = []
    prev_last_ts = None  # FIX #8: Track last timestamp to detect infinite loops
    ...
    while len(all_data) < limit:
        ...
        # FIX #8: Prevent infinite loop if no new data
        if raw[-1][0] == prev_last_ts:
            logger.warning(f"fetch {symbol} {timeframe}: no new data received")
            break

        prev_last_ts = raw[-1][0]
        since = raw[-1][0] + 1
        if len(raw) < batch:
            break
...
class DataManager:
    ...
    def _load_mini(self, symbol: str) -> bool:
        """
        Load MINI_BARS (50400) into mini store.

        WARM (кэш есть): incremental — 1-2 запроса, мгновенно.
          Если кэш короче MINI_BARS — добиваем backfill.
        COLD (кэша нет): полная загрузка 50400 баров через paged fetch.
          fetch_ohlcv_sync делает batch по 1000 баров = ~51 HTTP запрос.
          Rate-limit (INITIAL_RATE_RPS) управляется снаружи в load_all_history.
        """
        cached = cache_load(symbol, "1m")
        if cached is not None:
            # ── WARM PATH: есть кэш — incremental update ─────────────────────
                ...
                try:
                    ...
                    if missing <= 1000:
                        ...
                        if raw:
                            ...
                            cached = combined.iloc[-MINI_BARS:]
                            cache_save(symbol, "1m", cached)
                    else:
                        # Большой пропуск (>1000 баров, ~16ч+) — paged fetch
                        logger.info(f"mini large gap {symbol}: {missing} bars missing, fetching...")
                        df_new = fetch_ohlcv_sync(ex, symbol, "1m", min(missing + 10, MINI_BARS))
                        if not df_new.empty:
                            combined = pd.concat([cached, df_new])
                            combined = combined[~combined.index.duplicated(keep="last")]
                            combined.sort_index(inplace=True)
                            cached = combined.iloc[-MINI_BARS:]
                            cache_save(symbol, "1m", cached)
                            logger.info(f"mini gap filled {symbol}: now {len(cached)} bars")
                except Exception as e:
                    logger.warning(f"mini incremental {symbol}: {e}")

            # Если кэш короче MINI_BARS — добиваем backfill
            if len(cached) < MINI_BARS - 10:
                try:
                    ex      = self._make_sync_ex()
                    df_full = fetch_ohlcv_sync(ex, symbol, "1m", MINI_BARS)
                    if not df_full.empty:
                        ...
                        cached = combined.iloc[-MINI_BARS:]
                        cache_save(symbol, "1m", cached)
                        logger.info(f"mini backfilled: {symbol} ({len(cached)} bars)")
                except Exception as e:
                    logger.warning(f"mini backfill {symbol}: {e}")

            self.store.init_mini(symbol, cached)
            return True

        # ── COLD PATH: нет кэша — полная загрузка MINI_BARS баров ────────────
        # Rate-limit снаружи обеспечивает load_all_history (INITIAL_RATE_RPS).
        # fetch_ohlcv_sync делает ~51 последовательных batch-запроса по 1000 баров.
        try:
            ex = self._make_sync_ex()
            ...
            logger.info(f"mini cold loaded: {symbol} ({len(df)} bars)")
            return True
        except Exception as e:
            logger.error(f"load_mini cold {symbol}: {e}")
            return False
    ...
    def _load_chart(self, symbol: str) -> bool:
        ...
        if cached is not None:
            ...
            # ── FAST PATH: кэш свежий и полный — никакой сети, мгновенно ──────
            # WS уже пишет текущие бары в реальном времени, пропуск < 5 мин
            # при открытии графика незаметен пользователю.
            if not need_backfill and missing <= 5:
                logger.info(f"chart fast-cache: {symbol} ({len(cached)} bars, lag={missing}m)")
                self.store.init_chart(symbol, cached)
                return True

            # ── SLOW PATH: нужна сеть — берём exchange только здесь ──────────
            ex = self._make_sync_ex()

            if need_backfill:
                # Backfill: в кэше только мини-история (MINI_BARS < CHART_LIMIT)
                try:
                ...
                # Incremental: кэш полный, но пропущено > 5 минут
                try:
                    raw = ex.fetch_ohlcv(symbol, "1m", since=last_ts_ms,
        ...
        # Кэша нет совсем — полная загрузка с нуля
        ex = self._make_sync_ex()
        try:
    ...
    async def load_all_history(self, symbols: List[str]):
        """
        Startup: load MINI_BARS (50400) for all symbols.

        Два режима в зависимости от наличия кэша:
        - COLD START (нет кэша): строгий rate-limit INITIAL_RATE_RPS req/sec —
          не превышаем лимит Binance 2400 req/min.
          50400 баров = 51 пакет × ~1 запрос → при 4 rps ~13 сек на символ.
        - WARM START (есть кэш): параллельно MAX_WORKERS — только incremental,
          1-2 запроса на символ, быстро.
        """
        logger.info(f"Загрузка истории {len(symbols)} монет (MINI_BARS={MINI_BARS})...")

        cold = [s for s in symbols if cache_load(s, "1m") is None]
        warm = [s for s in symbols if s not in cold]

        logger.info(f"  Cold start (нет кэша): {len(cold)} монет")
        logger.info(f"  Warm start (есть кэш): {len(warm)} монет")

        done  = 0
        total = len(symbols)

        # ── WARM: параллельная incremental загрузка ──────────────────────────
        if warm:
            sem = asyncio.Semaphore(MAX_WORKERS)

            async def load_warm(sym):
                nonlocal done
                async with sem:
                    loop = asyncio.get_event_loop()
                    ...
                    if done % 50 == 0 or done == total:
                        logger.info(f"  {done}/{total} загружено (warm)")

            await asyncio.gather(*[load_warm(s) for s in warm])

        # ── COLD: строго rate-limited последовательная загрузка ──────────────
        # 1 воркер + sleep(interval) — гарантирует <= INITIAL_RATE_RPS req/sec
        if cold:
            interval = 1.0 / INITIAL_RATE_RPS
            logger.info(
                f"  Cold загрузка {len(cold)} монет @ {INITIAL_RATE_RPS} rps "
                f"(~{len(cold) / INITIAL_RATE_RPS / 60:.0f} мин)"
            )
            loop = asyncio.get_event_loop()
            for sym in cold:
                await loop.run_in_executor(None, self._load_mini, sym)
                ...
                if done % 50 == 0 or done == total:
                    logger.info(f"  {done}/{total} загружено (cold)")
                await asyncio.sleep(interval)

        self.store.ready = True
        logger.info(f"История загружена ✓ ({done}/{total} монет)")
            ...
            while self._running:
                try:
                    ...
                    for symbol, tf_data in ohlcvs.items():
                        # ccxt sometimes returns a DataFrame instead of dict — guard against it
