...
Логика:
  ...
  5. Находим ценовой уровень медианного qty через cumsum по отсортированным по цене уровням.
  6. Проверяем глубину стакана вокруг медианы (±depth уровней).
"""
...
def calc_dom_median(
    ...
    then find its price coordinate via cumsum and analyse ±depth levels around it.
    ...
    - median_price — ценовой уровень где накопленный qty пересекает медианный qty.

    Returns:
        {
          "median_qty":    float,            # робастная медиана qty (referens)
          "median_price":  float,            # ценовой уровень медианного qty
          "median_usd":    float,            # USD at median level
    ...
    all_levels.sort(key=lambda x: x[0])  # sort by price ascending

    # ── Robust qty-median (bottom 90%) — подход из main.py ───────────────────
    # Сортируем все qty, обрезаем топ 10% чтобы аномальные стены не тянули медиану
    zone_qtys = sorted(x[1] for x in all_levels)
    ...
    # ── Ценовой уровень медианного qty через cumsum по qty ────────────────────
    # Находим первый уровень где накопленный qty >= 50% суммарного qty в зоне
    qtys   = np.array([x[1] for x in all_levels])
    prices = np.array([x[0] for x in all_levels])

    total_qty  = qtys.sum()
    cumqty     = np.cumsum(qtys)
    median_idx = int(np.argmax(cumqty >= total_qty * 0.5))
    median_idx = min(median_idx, len(prices) - 1)
    median_price = float(prices[median_idx])
    median_usd   = float(all_levels[median_idx][2])  # price * qty at that level

    # ── Depth zone: ±depth уровней вокруг median_idx ─────────────────────────
    ...
    # Strongest wall near median (по USD)
