...
import secrets
import statistics
import threading  # FIX #6: For periodic cache cleanup
import time as _time
from collections import defaultdict
from contextlib import asynccontextmanager
from typing import List  # FIX #9: For type hints

import pandas as pd
import requests as _requests
import heapq  # FIX #4: For efficient top-k selection
from pydantic import BaseModel, validator  # FIX #9: For API validation
...
try:
    from dom_median import get_dom_median_full, SequentialDOMFetcher  # FIX #5: Import async fetcher
    _DOM_MEDIAN_AVAILABLE = True
except ImportError:
    _DOM_MEDIAN_AVAILABLE = False
    def get_dom_median_full(*a, **kw): return {"error": "dom_median.py not found"}
...
# ═══════════════════════════════════════════════════════════════════════════════
# FIX #9: PYDANTIC MODELS FOR API VALIDATION
# ═══════════════════════════════════════════════════════════════════════════════

class AbsorptionRequest(BaseModel):
    """FIX #9: Validate absorption request parameters"""
    symbol: str
    tf: str = "5m"
...
class POCRequest(BaseModel):
    """FIX #9: Validate POC request parameters"""
    symbol: str
    tf: str = "5m"
    show_4h: bool = True
    show_1d: bool = True
...
# ═══════════════════════════════════════════════════════════════════════════════
# FIX #6: PERIODIC CACHE CLEANUP
# ═══════════════════════════════════════════════════════════════════════════════

def _periodic_cache_cleanup():
    """FIX #6: Clean up expired cache entries every 60 seconds"""
    while True:
        _time.sleep(60)
        try:
            now = _time.monotonic()
            to_delete = [
                k for k, (_, ts) in _result_cache.items()
                if now - ts > _RESULT_TTL
            ]
            if to_delete:
                logger.debug(f"Cache cleanup: removing {len(to_delete)} stale entries")
                for k in to_delete:
                    del _result_cache[k]
...
_cleanup_thread = threading.Thread(target=_periodic_cache_cleanup, daemon=True)
...
# ══════════════════════════════════════════════════════════════════════════════
# SYMBOL FILTER — исключения для скринера Big Bid/Ask
# ══════════════════════════════════════════════════════════════════════════════

import re as _re

# Стейблкоины (полный список популярных)
_STABLECOINS = {
    "USDC", "BUSD", "TUSD", "USDP", "USDD", "GUSD", "FRAX", "LUSD",
    "SUSD", "CUSD", "EURS", "EURT", "EURC", "DAI", "MIM", "ALUSD",
    "CRVUSD", "PYUSD", "FDUSD", "USDE", "USDX", "USD0", "USDY",
}

# Паттерны для исключения
_EXCLUDE_DATE_RE   = _re.compile(r'\d{6}$')          # BTC-260327, ETH-250328 и т.п.
_EXCLUDE_HIRAGANA  = _re.compile(r'[\u3040-\u30ff\u4e00-\u9fff\uff00-\uffef]')  # иероглифы CJK

# Конкретные тикеры для исключения
_EXCLUDE_EXACT = {"BTCDOM", "DEFIUSDT", "ALTUSDT"}


def _should_exclude_symbol(symbol: str) -> bool:
    """
    Возвращает True если символ нужно исключить из скринера Big Bid/Ask:
    - Стейблкоины (USDC/USDT/DAI и т.д.)
    - Квартальные/датированные токены (BTC-260327, BTC_260328)
    - BTCDOM и подобные индексные токены
    - Символы с иероглифами
    """
    # Берём только базовый актив (до первого '/')
    ...
    # 2. Квартальные токены: содержат дату в формате YYMMDD в конце
    #    Примеры: BTC-260327, ETHUSDT-260328, BTC_250926
    clean = _re.sub(r'[-_]', '', base)  # убираем разделители
    if _EXCLUDE_DATE_RE.search(clean):
        return True
    # Также ловим формат с дефисом/подчёркиванием напрямую
    if _re.search(r'[-_]\d{6}$', base):
        return True
    ...
    # 4. Иероглифы (CJK, Hiragana, Katakana, Fullwidth)
    if _EXCLUDE_HIRAGANA.search(base):
        return True
...
async def api_absorption(
    ...
    """
    FIX #9: Parameters are validated by Pydantic model
    FIX #6: Results are cached with automatic cleanup every 60s
    """
    # FIX #9: Validate parameters
    try:
...
# ──────────────────────────────────────────────────────────────────────────────
_DOM_ANOMALIES_LOCK = asyncio.Lock()
...
async def _dom_poll_loop():
    from dom_median import SequentialDOMFetcher

    # Создаем fetcher с rate limiting
    # 0.67 req/sec = 2,400 req/hour (ровно лимит Binance)
    fetcher = SequentialDOMFetcher(rate=0.67)
    ...
    while True:
        try:
            ...
            # Конвертируем CCXT формат в Binance формат
            # BTC/USDT:USDT → BTCUSDT
            bsyms = []
            for sym in symbols:
                base = sym.split("/")[0]
                quote = sym.split("/")[1].split(":")[0]
                bsyms.append(base + quote)

            # ГЛАВНОЕ: fetcher.fetch_all делает ВСЕ запросы с rate limiting
            # Это занимает ~13 минут для 550 символов, но соблюдает лимиты
            logger.info(f"dom_poll: начинаем загрузку {len(symbols)} символов")
            doms = await fetcher.fetch_all(bsyms)
            logger.info(f"dom_poll: загрузка завершена")
            ...
            for sym, bsym in zip(symbols, bsyms):
                try:
                    ...
                    anomalies = _detect_anomalies(sym, cur_price, df_1m=df)
                    async with _DOM_ANOMALIES_LOCK:
                        _DOM_ANOMALIES[sym] = anomalies
        ...
        # Между циклами даем 5 минут перерыва
        # (fetcher уже потратил ~13 минут на загрузку)
        await asyncio.sleep(_DOM_POLL_INTERVAL)


@app.post("/api/set_limit_mult")
async def api_set_limit_mult(mult: float = 8.0):
    return await api_set_limit_params(mult=mult)


@app.post("/api/set_limit_params")
async def api_set_limit_params(mult: float = 8.0):
    """Only mult is user-configurable. depth and min_usd are server constants."""
...
async def api_anomalies():
    ...
    m      = get_manager()
    for sym in m.store.symbols:
        # Исключаем стейблы, квартальные токены, BTCDOM, иероглифы
        if _should_exclude_symbol(sym):
            continue
...
@app.get("/api/dom_median/{symbol:path}")
async def api_dom_median(
    symbol:     str,
    atr_period: int   = 100,
    atr_mult:   float = 5.0,
    dom_depth:  int   = 500,
    ...
    """
    ATR(period)*mult range on 5m TF → DOM median analysis within that range.
    depth — number of DOM levels above/below the median to analyse.
    """
    cache_key = ("dom_median", symbol, atr_period, atr_mult, dom_depth, depth)
    cached = _cache_get(cache_key)
    if cached is not None:
        return cached
    ...
    loop   = asyncio.get_event_loop()
    try:
        df_5m = await loop.run_in_executor(None, resample_from_1m, df_1m, "5m")
        if df_5m is None or df_5m.empty:
            raise HTTPException(404, f"Нет 5m данных {symbol}")
    except Exception as e:
        logger.error(f"resample failed for {symbol}: {e}")
        raise HTTPException(500, f"Ошибка обработки данных: {e}")

    result = await loop.run_in_executor(
        None, get_dom_median_full,
        symbol, df_5m, atr_period, atr_mult, dom_depth, depth
    )

    # Cache for 5 seconds — DOM is live data, don't over-cache
    cache_time = _time.monotonic()
    _result_cache[cache_key] = (result, cache_time)
    return result
