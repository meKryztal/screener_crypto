...
# _LEVEL_HISTORY удалён — персистентность проверяется через _DOM_FIRST_SEEN
...
def _detect_anomalies(symbol: str, current_price: float, df_1m=None) -> list:
    ...
    for side, levels in [("bid", bids), ("ask", asks)]:
        for price, qty in levels:
            ...
            # Персистентность через _DOM_FIRST_SEEN:
            # уровень должен присутствовать >= _DOM_MIN_PERSIST сек
            first_ts = fs.get(key)
            if first_ts is None:
                fs[key] = now
                continue
            if now - first_ts < _DOM_MIN_PERSIST:
                continue
