...
def _calc_absorption_vec(df: pd.DataFrame, thresh_series: pd.Series) -> pd.DataFrame:
    ...
    # FIX #3: Correct shadow detection logic
    # mid находится В ТЕНИ если она НЕ в теле свечи
    v_up = (mid >= top) & (mid <= h)  # ✓
    v_dn = (mid <= bot) & (mid >= l)  # ✓
    in_shadow = v_up | v_dn
...
def absorption_to_json(df: pd.DataFrame) -> list:
    ...
    # FIX #2: Convert nanoseconds to milliseconds correctly
    ts_ms   = (df.index.astype(np.int64) // 10**6).astype(np.int64).tolist()
    base_ms = (df["base_ts"].values.astype(np.int64) // 10**6).astype(np.int64).tolist()
...
def ohlcv_to_json(df: pd.DataFrame) -> list:
    """
    Оптимизировано: убран iterrows(), заменён на векторные операции.
    FIX #7: Use milliseconds consistently (not seconds)
    """
    if df is None or df.empty:
        return []

    # FIX #7: Convert to milliseconds (consistent with absorption_to_json)
    ts_ms = (df.index.astype(np.int64) // 10**6).astype(np.int64).tolist()
